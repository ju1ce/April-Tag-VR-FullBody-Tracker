...
void Tracker::CalibrateTracker()
{
    ...
    while (cameraRunning && mainThreadRunning)
    {
        ...
        cv::aruco::estimatePoseSingleMarkers(corners, static_cast<float>(markerSize), calib_config.camMat, calib_config.distortionCoeffs, rvecs, tvecs);
        ...
        for (int i = 0; i < boardIds.size(); i++) // for each of the trackers
        {
            ...
            try
            {
                if (cv::aruco::estimatePoseBoard(corners, ids, arBoard, calib_config.camMat, calib_config.distortionCoeffs, boardRvec[i], boardTvec[i], false) > 0) // try to estimate current trackers pose
                {
                    cv::aruco::drawAxis(image, calib_config.camMat, calib_config.distortionCoeffs, boardRvec[i], boardTvec[i], 0.1f); // if found, draw axis and mark it found
                    boardFound[i] = true;
                }
                else
                {
...
void Tracker::MainLoop()
{
    ...
    while (mainThreadRunning && cameraRunning) // run detection until camera is stopped or the start/stop button is pressed again
    {
        ...
        for (int i = 0; i < trackerNum; i++)
        {
            ...
            cv::projectPoints(point, rvec, tvec, calib_config.camMat, calib_config.distortionCoeffs, projected);
            ...
            if (tracker_pose_valid == 0) // if the pose from steamvr was valid, save the predicted position and rotation
            {
                ...
                cv::aruco::drawAxis(drawImg, calib_config.camMat, calib_config.distortionCoeffs, rvec, tvec, 0.10f);
        ...
        for (int i = 0; i < trackerNum; ++i)
        {
            ...
            try
            {
                ...
                if (cv::aruco::estimatePoseBoard(corners, ids, trackers[connection->connectedTrackers[i].TrackerId], calib_config.camMat, calib_config.distortionCoeffs, trackerStatus[i].boardRvec, trackerStatus[i].boardTvec, trackerStatus[i].boardFound && user_config.usePredictive) <= 0)
                {
                    for (int j = 0; j < 6; j++)
                    {
