...
void Tracker::CameraLoop()
{
    ...
    while (cameraRunning)
    {
        ...
        double timeSinceLast = std::chrono::duration<double>(curtime - last_preview_time).count();  //ensure that preview isnt shown more than 60 times per second, otherwise the CallAfter function gets overloaded
...
void Tracker::MainLoop()
{
    ...
    while (mainThreadRunning && cameraRunning)      //run detection until camera is stopped or the start/stop button is pressed again
    {
        ...
        if (manualRecalibrate)          //playspace calibration loop
        {
            ...
            {
                double timeSincePress = std::chrono::duration<double>(start - calibControllerLastPress).count();      //to prevent accidental double presses, 0.2 seconds must pass between presses.
